...
weight_file = os.getcwd() + f'\dqn_00.mat'
hist_file = '/home/babel/PycharmProjects/NOMA_with_reinformcement'
...
def Network(s, a, name):
 ...
 with tf.compat.v1.variable_scope(name + '.0', reuse=reuse):
    ...
    with tf.compat.v1.variable_scope(name + '.2', reuse = reuse):
        w,b = Variable([64, power_num])
        q_hat = tf.matmul(l, w) + b
   # r = tf.reduce_sum(tf.multiply(q_hat, a), reduction_indices = 1)  reduction_indices change to axis because it is deprecated
    r = tf.reduce_sum(tf.multiply(q_hat, a), axis = 1)
...
#def Network_ini(theta):
 #   update=[]
  #  for var in list_main:
      #  w=tf.Variable(tf.compat.v1.get_default_graph().get_tensor_by_name(var.name),tf.constant(np.reshape(theta[var.name],var.shape)))
   #     w=tf.Variable(tf.constant(np.reshape(theta[var.name],var.shape)))
         #b=tf.compat.v1.get_default_graph().get_tensor_by_name(var.name)

       # w.assign(tf.compat.v1.get_default_graph().get_tensor_by_name(var.name))
       # session=tf.compat.v1.Session()
      #  session.run(w)
        #tf.compat.v1.placeholder
        #session.run(w)
        #update.append(w)
    # update.append( session.run(w)
    #    update.append(tf.concat([tf.compat.v1.get_default_graph().get_tensor_by_name(var.name),tf.constant(np.reshape(theta[var.name], var.shape))],axis=1))
    # update.append(tf.assign(tf.get_default_graph().get_tensor_by_name(var.name),tf.constant(np.reshape(theta[var.name],var.shape))))
     ...
     #  update.append(tf.compat.v1.placeholder(tf.compat.v1.get_default_graph().get_tensor_by_name(var.name), tf.constant(np.reshape(theta[var.name], var.shape))))
    #  update.append(tf.compat.v1.assign(tf.compat.v1.get_default_graph().get_tensor_by_name(var.name),tf.constant(np.reshape(theta[var.name],var.shape))))
     # update.append(tf.compat.v1.assign(tf.compat.v1.get_default_graph().get_tensor_by_name(var.name), tf.constant(np.reshape(theta[var.name], var.shape))))
...
def Network_ini(theta):
    update=[]
    for var in list_main:
        w=tf.constant(np.reshape(theta[var.name],var.shape))
        #b = tf.compat.v1.get_default_graph().get_tensor_by_name(var.name)
        gd = tf.compat.v1.get_default_graph().as_graph_def()
        w, = tf.graph_util.import_graph_def(gd, return_elements=['main.0/main.0/w:0'])
        update.append(w)
...
def Train_episode(sess, episode): 
    ...
    for step_index in range(int(EPISODE)):
...
def Test_episode(sess, episode):
  ...
  #  y = list()
   ...
   # for k in range(5):
    ...
    PDDQN_list=Smooth(np.array(reward_dqn_list), window)
    PDDQN_Filler = np.std(PDDQN_list[0:EPISODE])
    DDQN_list=Smooth(np.array(reward_fp_list), window)
    DDQN_Filler = np.std(DDQN_list[0:EPISODE])
    DQN_list=Smooth(np.array(reward_rp_list), window)
    DQN_Filler = np.std(DQN_list[0:EPISODE])
    QN_list = Smooth(np.array(reward_wmmse_list), window)
    QN_Filler = np.std(QN_list[0:EPISODE])
    ax.plot(np.arange(EPISODE), PDDQN_list[0:EPISODE], color='red', label='DQN') #black PDDQN
    ax.fill_between(np.arange(EPISODE), PDDQN_list[0:EPISODE] - PDDQN_Filler, PDDQN_list[0:EPISODE]+ PDDQN_Filler, alpha=0.3, facecolor='red', )
#    ax.plot(range(EPISODE), reward_dqn_list[0:EPISODE], color='red', label='dqn_list')
    ax.plot(np.arange(EPISODE), DDQN_list[0:EPISODE], color='blue', label='DDQN') #blue DDQN
    ax.fill_between(np.arange(EPISODE), DDQN_list[0:EPISODE] - DDQN_Filler, DDQN_list[0:EPISODE]+ DDQN_Filler, alpha=0.3, facecolor='blue', )
    ax.plot(np.arange(EPISODE), DQN_list[0:EPISODE], color='Orange', label='QN ') #red DQN
    ax.fill_between(np.arange(EPISODE), DQN_list[0:EPISODE] - DQN_Filler, DQN_list[0:EPISODE]+ DQN_Filler, alpha=0.3, facecolor='Orange', )
    ax.plot(np.arange(EPISODE), QN_list[0:EPISODE], color='black', label='PDDQN') # Orange  QN
    ax.fill_between(np.arange(EPISODE), QN_list[0:EPISODE] - QN_Filler, QN_list[0:EPISODE]+ QN_Filler, alpha=0.3, facecolor='black', )
    #ax.plot(np.arange(EPISODE), Smooth(np.array(reward_mp_list), window)[0:EPISODE], color='green', label='mp_list')
    #////////////////////////////////////////////////////////////////
    #ax.plot(np.arange(EPISODE), PDDQN_list[0:EPISODE], color='black', label='PDDQN')
    #ax.fill_between(np.arange(EPISODE), PDDQN_list[0:EPISODE] - PDDQN_Filler, PDDQN_list[0:EPISODE] + PDDQN_Filler, alpha=0.3, facecolor='Orange', )
    #    ax.plot(range(EPISODE), reward_dqn_list[0:EPISODE], color='red', label='dqn_list')
    #ax.plot(np.arange(EPISODE), DDQN_list[0:EPISODE], color='blue', label='DDQN')
    #ax.fill_between(np.arange(EPISODE), DDQN_list[0:EPISODE] - DDQN_Filler, DDQN_list[0:EPISODE] + DDQN_Filler, alpha=0.3, facecolor='blue', )
    #ax.plot(np.arange(EPISODE), DQN_list[0:EPISODE], color='red', label='DQN')
    #ax.fill_between(np.arange(EPISODE), DQN_list[0:EPISODE] - DQN_Filler, DQN_list[0:EPISODE] + DQN_Filler, alpha=0.3,  facecolor='red', )
    #ax.plot(np.arange(EPISODE), QN_list[0:EPISODE], color='Orange', label='QN')
    #ax.fill_between(np.arange(EPISODE), QN_list[0:EPISODE] - QN_Filler, QN_list[0:EPISODE] + QN_Filler, alpha=0.3, facecolor='Orange', )
    # ax.plot(np.arange(EPISODE), Smooth(np.array(reward_mp_list), window)[0:EPISODE], color='green', label='mp_list')
   # plt.legend(loc=7)
    plt.legend(ncol=1, loc='lower right')  # loc='upper right'bbox_to_anchor=(1,0.5)
    plt.xlabel('Episodes',fontdict={'fontsize':20,})
    plt.ylabel('Average rewards',fontdict={'fontsize':20,})
    #plt.ylim((20, 140))
    #plt.ylim((0))
    plt.xlim(0, EPISODE)
    plt.savefig('EpisodeRewards.png')
    #plt.grid()
    plt.show()
    ...
    # fig, ax = plt.subplots(1, 1)
    #x = np.arange(200)
    #ax.plot(x, average_r[0:200], color='red', label='ABDRL')
 #   ax.fill_between(x, average_r[0:200] - std_ABDRL, average_r[0:200] + std_ABDRL, alpha=0.15, facecolor='red', )
    #  train_hist = Smooth(np.array(train_hist), window)
    #  plt.figure(4)
    # plt.plot(range(EPISODE), train_hist)
    #  plt.plot(range(EPISODE), train_hist)
...
#///////////////////////////
  ...
  #  plt.figure(3)
   # window = 11
    #window = 101
  #  y = list()
  #  y.append(Smooth(np.array(dqn_hist), window))
   # y.append(Smooth(np.array(fp_hist), window))
   # y.append(Smooth(np.array(wmmse_hist), window))
    #y.append(Smooth(np.array(mp_hist), window))
   # y.append(Smooth(np.array(rp_hist), window))
   # label = ['DQN', 'FP', 'WMMSE', 'Random power', 'Maximal power']
   # color = ['royalblue', 'orangered', 'lawngreen', 'gold', 'olive']
   # linestyle = ['-', '--', '-.', ':', '--']
   # p = list()
   ...
   # fig, ax = plt.subplots(1, 1)
#    ax.plot(np.arange(EPISODE), Smooth(np.array(dqn_hist), window)[0:EPISODE], color='red', label='dqn_list1')
    #ax.plot(range(EPISODE), reward_dqn_list[0:EPISODE], color='red', label='dqn_list')
  #  ax.plot(np.arange(EPISODE), Smooth(np.array(fp_hist), window)[0:EPISODE], color='blue', label='fp_list1')
   # ax.plot(np.arange(EPISODE), Smooth(np.array(wmmse_hist), window)[0:EPISODE], color='red', label='wmmse_list1')
  #  ax.plot(np.arange(EPISODE), Smooth(np.array(mp_hist), window)[0:EPISODE], color='green', label='mp_list1')
  #  ax.plot(np.arange(EPISODE), Smooth(np.array(rp_hist), window)[0:EPISODE], color='black', label='rp_list1')
 #   plt.legend(loc=7)
  #  plt.xlabel('Episodes1')
  #  plt.ylabel('Average rewards1')
 #   plt.grid()
 #   plt.show()
     ...
     #   y = list()
      ...
      #  p = list()
       # for k in range(5):
        ...
        #for k in range(100):
           # p_temp, = plt.plot(range(int(EPISODE)), y[k], color=color[k], linestyle=linestyle[k], label=label[k])
           # p.append(p_temp)


        fig, ax = plt.subplots(1, 1)
     #   ax.plot(np.arange(EPISODE), Smooth(np.array(reward_dqn_list), window)[0:EPISODE], color='royalblue', label='DQN')
      # ax.plot(range(EPISODE), reward_dqn_list[0:EPISODE], color='red', label='dqn_list')
      #  ax.plot(np.arange(EPISODE), Smooth(np.array(reward_fp_list), window)[0:EPISODE], color='orangered', label='FP')
      #  ax.plot(np.arange(EPISODE), Smooth(np.array(reward_wmmse_list), window)[0:EPISODE], color='lawngreen', label='WMMSE')
      #  ax.plot(np.arange(EPISODE), Smooth(np.array(reward_mp_list), window)[0:EPISODE], color='gold', label='Random power')
      #  ax.plot(np.arange(EPISODE), Smooth(np.array(reward_rp_list), window)[0:EPISODE], color='olive', label='Maximal power')
       # plt.legend(loc = 7)
       # plt.xlabel('Episodes')
       ...
       # plt.show()
        ...
        QN_Filler = np.std(QN_list[0:EPISODE])
        ax.plot(np.arange(EPISODE), PDDQN_list[0:EPISODE], color='black', label='PDDQN')
        ax.fill_between(np.arange(EPISODE), PDDQN_list[0:EPISODE] - PDDQN_Filler, PDDQN_list[0:EPISODE] + PDDQN_Filler, alpha=0.3, facecolor='black', )
 #    ax.plot(range(EPISODE), reward_dqn_list[0:EPISODE], color='red', label='dqn_list')
        ax.plot(np.arange(EPISODE), DDQN_list[0:EPISODE], color='red', label='DQN') # blue DDQN
        ax.fill_between(np.arange(EPISODE), DDQN_list[0:EPISODE] - DDQN_Filler, DDQN_list[0:EPISODE] + DDQN_Filler, alpha=0.3, facecolor='red', )
        ax.plot(np.arange(EPISODE), DQN_list[0:EPISODE], color='blue', label='DDQN') # red DQN
        ax.fill_between(np.arange(EPISODE), DQN_list[0:EPISODE] - DQN_Filler, DQN_list[0:EPISODE] + DQN_Filler, alpha=0.3, facecolor='blue', )
        ax.plot(np.arange(EPISODE), QN_list[0:EPISODE], color='Orange', label='QN')
        ax.fill_between(np.arange(EPISODE), QN_list[0:EPISODE] - QN_Filler, QN_list[0:EPISODE] + QN_Filler, alpha=0.3, facecolor='Orange', )
 # ax.plot(np.arange(EPISODE), Smooth(np.array(reward_mp_list), window)[0:EPISODE], color='green', label='mp_list')
     #   plt.legend(loc=7)
...
#   plt.legend(loc=7)
        ...
        plt.legend(ncol=1, loc='lower right')  # loc='upper right'bbox_to_anchor=(1,0.5)
        plt.xlabel('Episodes',fontdict={'fontsize':20,})
        plt.ylabel('Interference',fontdict={'fontsize':20,})
        plt.ylim((0.2, 1.6))
        plt.xlim(0, EPISODE)
        plt.savefig('interference.png')
        plt.show()

        print("Test: DQN: %.2f  FP: %.2f  WMMSE: %.2f  MP: %.2f  RP: %.2f" %(np.mean(reward_dqn_list), np.mean(reward_fp_list), np.mean(reward_wmmse_list), np.mean(reward_mp_list), np.mean(reward_rp_list)))
...
def Bar_plot():
    ...
    plt.figure(3)
    window = 5
    data_K1_list = [3.19, 2.97, 2.91, 2.00, 1.99]
    data_K2_list = [2.22, 1.97, 2.01, 1.02, 1.02]
 #   data_K3_list = [1.53, 1.33, 1.38, 0.51, 0.51]
 #   data_K4_list = [1.22, 1.05, 1.10, 0.35, 0.35]
  #  data_K1_list = [3.19, 2.97, 2.91]
   # data_K2_list =[2.22, 1.97, 2.01]
   # data_K3_list=[1.53, 1.33, 1.38]
   # data_K4_list=[1.22, 1.05, 1.10]
   # data_K1_list = np.array(data_K1_list).T
   # data_K2_list = np.array(data_K2_list).T
   # data_K3_list = np.array(data_K3_list).T
   # data_K4_list = np.array(data_K4_list).T
  #  fig, ax = plt.subplots(1, 1)
   # ax.plot(np.arange(5), Smooth(np.array(data_K1_list), window)[0:5], color='red', label='K1')
   # ax.plot(np.arange(5), Smooth(np.array(data_K2_list), window)[0:5], color='blue', label='K2')
   # ax.plot(np.arange(5), Smooth(np.array(data_K3_list), window)[0:5], color='red', label='K3')
   # ax.plot(np.arange(5), Smooth(np.array(data_K4_list), window)[0:5], color='green', label='K4')
   # plt.legend(loc=7)
   # plt.xlabel('User number per cell')
   # plt.ylabel('Average rate (bps)')
   # plt.grid()
...
#weight_file = os.getcwd() + f'\dqn_00.mat'
#hist_file = '/home/babel/PycharmProjects/NOMA_with_reinformcement'
#from scipy.io import savemat     savemat(custom_dir + f'\connected_users.mat', mdict_2)
